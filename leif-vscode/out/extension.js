...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('leifSidebar', provider));
...
class LeifSidebarProvider {
    ...
    async runAgentLoop(task, webview) {
        let history = [];
        let loopCount = 0;
...
Output format:
{
    ...
    "args": { ... }
            ...
            try {
                ...
                if (!ollamaRes.ok)
                    throw new Error("Ollama is offline or unreachable.");
                ...
                try {
                    ...
                    if (models.length === 0)
                        throw new Error("No active LMs in VS Code!");
            ...
            if (action.tool === "consult_copilot") {
                ...
                try {
                    ...
                    if (models.length > 0) {
                        ...
                        const chatResponse = await model.sendRequest([vscode.LanguageModelChatMessage.User(copilotPrompt)], {}, new vscode.CancellationTokenSource().token);
                        let advice = "";
                        for await (const chunk of chatResponse.text)
                            advice += chunk;
                ...
                try {
                    ...
                    if (models.length > 0) {
                        ...
                        const chatResponse = await model.sendRequest([vscode.LanguageModelChatMessage.User(premiumPrompt)], {}, new vscode.CancellationTokenSource().token);
                        let content = "";
                        for await (const chunk of chatResponse.text)
                            content += chunk;
                ...
                try {
                    const execResult = await new Promise((resolve) => {
                        ...
                        const child = (0, child_process_1.exec)(action.args.command, { cwd: cwdPath }, (error, stdout, stderr) => {
                            clearTimeout(timer);
                            if (error) {
                                resolve(`ERROR:\\n${error.message}\\nSTDERR:\\n${stderr}\\nSTDOUT:\\n${stdout}`);
                            }
                ...
                try {
                    ...
                    if (action.args.start_line !== undefined && action.args.end_line !== undefined) {
                        ...
                        if (start > 0)
                            finalContent = `...(Lines 1 to ${start} hidden)...\\n` + finalContent;
                        if (end < lines.length)
